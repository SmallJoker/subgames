local gravity = -9.81
if minetest.setting_get("movement_gravity") then
	gravity = minetest.setting_get("movement_gravity")*-1
end
local charge_speed = 0.5
local arrow_lifetime = 60
if minetest.setting_get("bow_lifetime_arrow") then
	arrow_lifetime = tonumber(minetest.setting_get("bow_lifetime_arrow"))
end
	...
	on_step = function(self, dtime)
		...
		elseif self.charge==0 then
			...
			for _,obj in ipairs(objects) do
				if obj:is_player() and minetest.get_player_privs(obj:get_player_name()).interact then
